    ...
    public class ToolsController : Controller
    {
        ...
        public async Task<IActionResult> AnalyzeMFT(IFormFile csvFile)
        {
            try
            {
                ...
                // ADD THIS: Generate report ID
                var reportId = $"RPT-MFT-{DateTime.Now:yyyyMMdd}-{Guid.NewGuid().ToString().Substring(0, 4).ToUpper()}";

                return Json(new
                {
                    success = true,
                    report_id = reportId, // ADD THIS LINE
                    case_id = caseId,
                    user_id = userId,
        ...
        /// </summary>
        /// 
        // Add this method to generate text reports
        private string GenerateTextReport(dynamic result, string caseId, string userId)
        {
            var sb = new StringBuilder();

            sb.AppendLine("================================================");
            sb.AppendLine("           MFT ANALYSIS REPORT");
            sb.AppendLine("================================================");
            sb.AppendLine();
            sb.AppendLine($"Case ID: {caseId}");
            sb.AppendLine($"User ID: {userId}");
            sb.AppendLine($"Analysis Date: {result.date_created}");
            sb.AppendLine($"Report ID: {result.report_id}");
            sb.AppendLine();

            sb.AppendLine("RISK ASSESSMENT:");
            sb.AppendLine($"  Score: {result.analysis.score}/100");
            sb.AppendLine($"  Alert Level: {result.analysis.alert_level}");
            sb.AppendLine($"  Risk Color: {result.analysis.alert_color}");
            sb.AppendLine();

            sb.AppendLine("METRICS:");
            sb.AppendLine($"  Total Files Analyzed: {result.analysis.metrics.total_files}");
            sb.AppendLine($"  Large Timestamp Discrepancies: {result.analysis.metrics.large_timestamp_discrepancies}");
            sb.AppendLine($"  Large Timestamp Percentage: {result.analysis.metrics.large_timestamp_percentage}%");
            sb.AppendLine($"  Rapid Deletions: {result.analysis.metrics.rapid_deletions}");
            sb.AppendLine($"  Rapid Deletion Percentage: {result.analysis.metrics.rapid_deletion_percentage}%");
            sb.AppendLine();

            if (!string.IsNullOrEmpty(result.ai_insights))
            {
                sb.AppendLine("AI ANALYSIS INSIGHTS:");
                sb.AppendLine("================================================");
                sb.AppendLine(result.ai_insights);
                sb.AppendLine();
            }

            sb.AppendLine("TOP SUSPICIOUS FILES:");
            sb.AppendLine("================================================");
            if (result.analysis.top_suspicious_files is IEnumerable<object> suspiciousFiles)
            {
                foreach (var file in suspiciousFiles)
                {
                    var fileDict = System.Text.Json.JsonSerializer.Deserialize<Dictionary<string, object>>(file.ToString());
                    if (fileDict != null)
                    {
                        var fileName = fileDict.ContainsKey("file_name") ? fileDict["file_name"]?.ToString() : "Unknown";
                        var yearsDelta = fileDict.ContainsKey("years_delta") ? fileDict["years_delta"]?.ToString() : "0";
                        sb.AppendLine($"  {fileName} (Delta: {yearsDelta} years)");
                    }
                }
            }
            sb.AppendLine();
            sb.AppendLine("================================================");
            sb.AppendLine("Report generated by Forensic Analysis Tool");
            sb.AppendLine("================================================");

            return sb.ToString();
        }

        // Add this action method for downloading text reports
        [HttpPost]
        public IActionResult DownloadTextReport([FromBody] JsonElement analysisResult)
        {
            try
            {
                // Extract properties safely from JsonElement
                var reportId = analysisResult.GetProperty("report_id").GetString();
                var caseId = analysisResult.GetProperty("case_id").GetString();
                var userId = analysisResult.GetProperty("user_id").GetString();
                var dateCreated = analysisResult.GetProperty("date_created").GetString();
                var fileName = analysisResult.GetProperty("file_name").GetString();
                var fileSize = analysisResult.GetProperty("file_size").GetInt64();
                var aiInsights = analysisResult.GetProperty("ai_insights").GetString();

                var analysis = analysisResult.GetProperty("analysis");
                var score = analysis.GetProperty("score").GetDouble();
                var alertLevel = analysis.GetProperty("alert_level").GetString();
                var alertColor = analysis.GetProperty("alert_color").GetString();

                var metrics = analysis.GetProperty("metrics");
                var totalFiles = metrics.GetProperty("total_files").GetInt32();
                var largeTimestampDiscrepancies = metrics.GetProperty("large_timestamp_discrepancies").GetInt32();
                var largeTimestampPercentage = metrics.GetProperty("large_timestamp_percentage").GetDouble();
                var rapidDeletions = metrics.GetProperty("rapid_deletions").GetInt32();
                var rapidDeletionPercentage = metrics.GetProperty("rapid_deletion_percentage").GetDouble();
...
RISK ASSESSMENT:
  ...
  Risk Color: {alertColor}

METRICS:
  Total Files Analyzed: {totalFiles}
  Large Timestamp Discrepancies: {largeTimestampDiscrepancies}
  Large Timestamp Percentage: {largeTimestampPercentage}%
  Rapid Deletions: {rapidDeletions}
  Rapid Deletion Percentage: {rapidDeletionPercentage}%
...
===============================================
Report generated by Forensic Analysis Tool
===============================================
";

                var bytes = Encoding.UTF8.GetBytes(textContent);
                return File(bytes, "text/plain", $"MFT_Report_{reportId}.txt");
            }
            catch (Exception ex)
            {
                return BadRequest($"Failed to generate text report: {ex.Message}");
            }
        }
